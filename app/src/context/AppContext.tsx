...
interface AppContextType {
  ...
  createProduct: (product: { name: string; price: number; stock: number; emoji: string }) => Promise<void>;
}
...
export function AppProvider({ children }: { children: React.ReactNode }) {
  ...
  const fetchTransactions = useCallback(async () => {
    try {
      ...
      // Transform the response to match our Transaction type
      const transactions = response.data.map((tx: any) => ({
        id: tx.id.toString(),
        productId: tx.details?.[0]?.product_id?.toString() || '',
        productName: tx.details?.[0]?.product?.name || '',
        type: tx.type,
        qty: tx.details?.reduce((sum: number, d: any) => sum + d.qty, 0) || 0,
        totalPrice: tx.grand_total,
        note: tx.note || '',
  ...
  const createProduct = useCallback(async (product: { name: string; price: number; stock: number; emoji: string }) => {
    try {
  ...
  useEffect(() => {
    ...
    if (state.user) {
      // Define fetch inline to avoid dependency chain issues
      (async () => {
        try {
